...
export function MapPopupSingle({
  ...
  return (
    <div className={`p-3 min-w-[300px] ${theme === "dark" ? "bg-[#1a1a1a] text-white" : "bg-white text-black"}`}>
      ...
      {/* Expanded Details - ALWAYS RENDERED to preserve scroll, hidden with display:none */}
      <div
        ref={scrollContainerRef}
        className="mt-3 pt-3 border-t border-border space-y-3 overflow-y-auto expanded-details"
        style={{
      ...
      >
          ...
          {onNavigate && (
            <div className="pt-3 mt-3 border-t border-border mb-2">
              <button
                onClick={(e) => {
                  e.stopPropagation()
          ...
          {/* Bottom spacer to ensure last content is scrollable */}
          <div className="h-4" />
        </div>
