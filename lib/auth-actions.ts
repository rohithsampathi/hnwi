...
// handleLogin is now DEPRECATED - components should call /api/auth/login directly
// This function exists only for backward compatibility and should not be used
export async function handleLogin(loginData: LoginData): Promise<AuthResponse> {
  logger.warn("DEPRECATED: handleLogin called - components should call /api/auth/login directly");
  return { success: false, error: "This function is deprecated. Please use the API route directly." };
}
