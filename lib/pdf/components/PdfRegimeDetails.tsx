...
export const PdfRegimeDetails: React.FC<PdfRegimeDetailsProps> = ({
  ...
  const th = { flex: 1 as const, fontFamily: 'Inter' as const, fontWeight: 700 as const, fontSize: 8.5, color: darkTheme.textPrimary, textTransform: 'uppercase' as const, letterSpacing: 0.5 };
  const priorityMap = (p: string) => {
    const h = p.toLowerCase() === 'high';
    return { color: h ? colors.amber[500] : darkTheme.textMuted, bg: h ? 'rgba(212,168,67,0.15)' : darkTheme.surfaceBg, border: h ? colors.amber[500] : darkTheme.border };
  };
  const AlertBox: React.FC<{ bg: string; border: string; icon: string; iconBg: string; title: string; titleColor: string; children: React.ReactNode }> = ({ bg, border, icon, iconBg, title, titleColor, children }) => (
    <View style={{ backgroundColor: bg, borderWidth: 1.5, borderColor: border, padding: 16, marginBottom: 24, flexDirection: 'row' }}>
      <View style={{ width: 24, height: 24, backgroundColor: iconBg, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
        <Text style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 14, color: darkTheme.pageBg }}>{icon}</Text>
      </View>
      <View style={{ flex: 1 }}>
  ...
  <>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
          {keyBenefits.slice(0, 6).map((b, i) => (
            <View key={i} style={{ width: '48%', flexDirection: 'row', alignItems: 'flex-start', backgroundColor: darkTheme.cardBg, borderWidth: 1, borderColor: darkTheme.border, padding: 12, marginRight: 8, marginBottom: 8 }} wrap={false}>
              <View style={{ width: 16, height: 16, backgroundColor: 'rgba(16,185,129,0.15)', borderRadius: 8, alignItems: 'center', justifyContent: 'center', marginRight: 8 }}>
                <Text style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 10, color: colors.emerald[400] }}>{'\u2713'}</Text>
              </View>
        ...
        <View style={{ marginBottom: 24, borderWidth: 1, borderColor: darkTheme.border }} wrap={false}>
          ...
          {qualificationRoutes.map((r, i) => (
            <View key={i} style={{ flexDirection: 'row', paddingVertical: 12, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: darkTheme.border, backgroundColor: i % 2 === 0 ? darkTheme.pageBg : darkTheme.cardBg }}>
              ...
              <Text style={{ flex: 1, fontFamily: 'Inter', fontWeight: 700, fontSize: 10, color: colors.emerald[400] }}>{r.processing_time}</Text>
            </View>
          ))}
        </View>
      </View>
          ...
          return (
            <View key={i} style={{ flexDirection: 'row', alignItems: 'flex-start', padding: 12, marginBottom: 8, borderWidth: 1, borderColor: darkTheme.border, backgroundColor: darkTheme.cardBg }} wrap={false}>
              <View style={{ paddingHorizontal: 8, paddingVertical: 3, marginRight: 12, backgroundColor: pm.bg, borderWidth: 1, borderColor: pm.border }}>
                <Text style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 8.5, textTransform: 'uppercase', letterSpacing: 0.5, color: pm.color }}>{c.priority}</Text>
              </View>
              <View style={{ flex: 1 }}>
    ...
    {applicationProcess && applicationProcess.length > 0 && (
      <View style={{ marginBottom: 24 }}>
        ...
        {applicationProcess.slice(0, 5).map((s, i) => (
          <View key={i} style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 8, padding: 12, backgroundColor: darkTheme.cardBg, borderWidth: 1, borderColor: darkTheme.border }}>
            <View style={{ width: 24, height: 24, backgroundColor: colors.amber[500], borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
              <Text style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 10, color: darkTheme.pageBg }}>{s.step}</Text>
            </View>
    ...
    {actionRequired && (
      <AlertBox bg="rgba(212,168,67,0.08)" border={colors.amber[500]} icon="!" iconBg={colors.amber[500]} title="Action Required" titleColor={colors.amber[500]}>
        <Text style={bodyText}>{actionRequired}</Text>
      </AlertBox>
    )}

    {successorRegime && (
      <AlertBox bg="rgba(16,185,129,0.08)" border={colors.emerald[500]} icon=">" iconBg={colors.emerald[500]} title="Successor Program" titleColor={colors.emerald[400]}>
    ...
    {regimeWarnings && regimeWarnings.length > 0 && (
      <View style={{ marginTop: 16 }}>
        {regimeWarnings.map((w, i) => (
          <AlertBox key={i} bg="rgba(239,68,68,0.08)" border={colors.red[500]} icon="!" iconBg={colors.red[500]} title="Warning" titleColor={colors.red[400]}>
