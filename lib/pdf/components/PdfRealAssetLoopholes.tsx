  ...
  return (
    <View style={{ marginBottom: 20 }}>
      ...
      {strategies.slice(0, 4).map((strategy, idx) => (
        <View key={idx} style={{ marginBottom: 12, backgroundColor: darkTheme.cardBg, borderWidth: 1, borderColor: darkTheme.border, borderLeftWidth: 4, borderLeftColor: colors.amber[500], padding: 14, position: 'relative', overflow: 'hidden' }} wrap={false}>
          {/* Corner accent */}
          <View style={{ position: 'absolute', top: 0, right: 0, width: 60, height: 60, backgroundColor: colors.tints.goldLight, borderRadius: 0.01 }} />
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10, position: 'relative', zIndex: 1 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
              <View style={{ width: 24, height: 24, backgroundColor: colors.amber[500], borderRadius: 0.01, alignItems: 'center', justifyContent: 'center', marginRight: 10 }}>
                <Text style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 11, color: darkTheme.contrastText }}>{idx + 1}</Text>
              </View>
            ...
            <Text style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 16, color: colors.amber[500] }}>{strategy.tax_savings_potential}</Text>
          </View>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 8 }}>
              {strategy.requirements.slice(0, 3).map((req, i) => (
                <View key={i} style={{ backgroundColor: darkTheme.surfaceBg, borderWidth: 1, borderColor: darkTheme.border, paddingHorizontal: 8, paddingVertical: 4, marginRight: 6, marginBottom: 6 }}>
                  <Text style={{ fontFamily: 'Inter', fontSize: 9, color: darkTheme.textMuted }}>{req}</Text>
                </View>
              ))}
            </View>
          )}
